...
def push_interfaces_to_nautobot(device_id, interfaces, token):
    ...
    nautobot_url = "http://192.168.0.44:8001/api/dcim/interfaces/"
    headers = {
    ...
    for interface_name, interface_data in interfaces.items():
        ...
        if untagged_vlan:
            interface_payload["untagged_vlan"] = {
                ...
                "url": f"http://192.168.0.44:8001/api/ipam/vlans/{untagged_vlan.get('id')}/"
            }
            ...
            interface_payload["tagged_vlans"] = [
                {
                    "id": vlan.get("id"),
                    "object_type": "ipam.vlan",
                    "url": f"http://192.168.0.44:8001/api/ipam/vlans/{vlan.get('id')}/"
                }
        ...
        try:
            check_url = f"http://192.168.0.44:8001/api/dcim/interfaces/?device_id={device_id}&name={interface_name}"
            response = requests.get(check_url, headers=headers)
            if response.status_code == 200 and response.json()["count"] > 0:
                # Interface exists, perform PATCH
                interface_id = response.json()["results"][0]["id"]
                patch_url = f"http://192.168.0.44:8001/api/dcim/interfaces/{interface_id}/"
    ...
    for vlan_id, vlan_data in vlans.items():
        ...
        try:
            response = requests.post(
                "http://192.168.0.44:8001/api/ipam/vlans/",
                headers={
                    "Authorization": f"Token {token}",
                    "Content-Type": "application/json",
                },
